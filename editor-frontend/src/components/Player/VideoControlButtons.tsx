...
const VideoControlButtons = ({ startTime, endTime, setStartTime, setEndTime, duration, setVideoID, loggedIn, originalVideoTitle }: ControlProps) => {
  ...
  const sendAPIRequest = async () => {
    ...
    if(title != originalVideoTitle && title != '') {
      const headers = {
        "Content-Type": "application/json",
      ...
      fetch("https://api-moments.testngg.net/video/title", {
        method: 'POST',
        headers: headers,
  ...
  return (
    <VideoControlsWrapper className="flex">
      ...
      <div className="reset-save-options flex">
        ...
        <button className={`save-btn flex ${title==originalVideoTitle && saveBtnActive}`} onClick={sendAPIRequest}>
          Save
